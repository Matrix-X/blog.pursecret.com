...
if ( ! class_exists( 'Blog_Elite_About' ) ) {
	...
	class Blog_Elite_About {
		 ...
		 * Single instance.
		 *
		 * @var Blog_Elite_About $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Blog_Elite_About ) ) {
				self::$instance = new Blog_Elite_About;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( 'About %s', 'blog-elite' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( 'About %s', 'blog-elite' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-about';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'blog-elite' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'blog-elite' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'blog_elite_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'blog-elite' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_blog_elite_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_blog_elite_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'blog-elite' ), $method );
			}
		}
		...
		public function useful_plugins() {
			...
			<div class="saga-section saga-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'blog-elite' ); ?></a>
				<?php endif; ?>
        ...
        public function free_vs_pro() {
            ...
            <div class="saga-section saga-section-free-vs-pro">
                <table class="free-vs-pro-table">
                    ...
                    <tr>
                        <th></th>
                        <th><?php _e('Free', 'blog-elite')?></th>
                        <th><?php _e('Pro', 'blog-elite')?></th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php
                    if(is_array($content)){
                        foreach ( $content as $item ) {
                            ...
                            <tr>
                                ...
                                <td class="free-options"><?php echo isset($item['free_text']) ? wp_kses_post( $item['free_text'] ) : '';?></td>
                                <td class="pro-options"><?php echo isset($item['pro_text']) ? wp_kses_post( $item['pro_text'] ) : '';?></td>
                            </tr>
                            <?php
                        }
                    }
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'blog-elite-about', get_template_directory_uri() . '/lib/about/css/about' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'blog-elite-about', get_template_directory_uri() . '/lib/about/js/about' . $min . '.js', array(), '1.0.0', true );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'blog_elite_dismiss_status', true );
			?>
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'blog_elite_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'blog_elite_dismiss_status', 1 );
